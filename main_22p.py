...
def myprior(cube):
    ...
    cube[15] = cube[15] * 2. - 1.
    cube[16] = cube[16] 
    cube[17] = cube[17] * radians(180.)
    cube[18] = cube[18] * radians(360.)+radians(180.)
    cube[19] = cube[19] * radians(360.)
    cube[20] = cube[20] * (p_mass_max3 - p_mass_min3) + p_mass_min3
    cube[21] = cube[21] * (period_max3 - period_min3) + period_min3
...
def gen_dx_dy_mc_3p(x, time_con):
    ...
    as_cal_con3 = GenModelPos_oneplanet(x[16], x[15], x[17], x[18], x[19], x[21], x[20], ms_Msun, d_pc, 0., 0., time_con)

    as_cal_con = as_cal_con1 + as_cal_con2 + as_cal_con3
...
def myloglike(cube):
    modelNow = gen_dx_dy_mc_3p(cube,time_con)
...
np.savetxt(f"{prefix}result_22p.dat", result['samples'])
...
as_cal_con_fit = DataGenFunc.GenModelPos_oneplanet(p_fit[1], p_fit[0], p_fit[2], p_fit[3]+radians(180.), p_fit[4], p_fit[6], p_fit[5], ms_Msun, d_pc, RA0, DEC0, time_con)
as_cal_con_fit1 = DataGenFunc.GenModelPos_oneplanet(p_fit[8], p_fit[7], p_fit[9], p_fit[10]+radians(180.), p_fit[11], p_fit[13], p_fit[12], ms_Msun, d_pc, 0., 0., time_con)
as_cal_con_fit2 = DataGenFunc.GenModelPos_oneplanet(p_fit[15], p_fit[14], p_fit[16], p_fit[17]+radians(180.), p_fit[18], p_fit[20], p_fit[19], ms_Msun, d_pc, 0., 0., time_con)
as_mu_fit_p = np.transpose(as_cal_con_fit + as_cal_con_fit1 + as_cal_con_fit2)
as_mu_fit = np.zeros((N_time-1,2),dtype=np.float64)
as_mu_fit[:,0] = as_mu_fit_p[1:N_time,0] - as_mu_fit_p[0,0]
as_mu_fit[:,1] = as_mu_fit_p[1:N_time,1] - as_mu_fit_p[0,1]
i_save_name =  "%s%s%s" % ("as_mu_fit_N_", int(i_tofit), ".dat")
np.savetxt(i_save_name, np.transpose([as_mu_fit[:,0],as_mu_fit[:,1]]))
res_N = np.zeros((N_time-1, 2), dtype= np.float64)
res_N[:,0] = delta_dx_dy_onerefstar[:,0] - as_mu_fit[:,0] 
res_N[:,1] = delta_dx_dy_onerefstar[:,1] - as_mu_fit[:,1]

#%%
# 1
clp_nra_1 = pyPeriod.Gls((time_con[1:N_time], res_N[:,0]))
clp_nra_1.info()
clp_nra_list_1 = (clp_nra_1.power).tolist()
max_ra_index_1 = clp_nra_list_1.index(max(clp_nra_list_1))
clp_ndec_1 = pyPeriod.Gls((time_con[1:N_time], res_N[:,1]))
clp_ndec_1.info()
...
clp_fig_1.savefig("LombScargle-2-22p.png", dpi=300)
...
figure1.savefig(f"Omege_omege_corner_chain{str(i_tofit)}_7.png", dpi=300)
plt.savefig(f'Omege_omege_corner_chain{str(i_tofit)}_7.pdf')
